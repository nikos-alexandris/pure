...
Definition eval_to'_def:
  ...
  (result_map' st xs =
    with_fst REVERSE (thread_state st xs)) ∧
...
Proof
  ...
  >>~- ([`mapok _ _`], 
        ...
        >> gvs [AllCaseEqs(), with_fst_def]
        >> imp_res_tac mapok_non_incr_lemma
        >> gvs [MAP_SND_MAP_WITH_FST_thm])
  (* result_map' body *)
  >>~- ([`with_fst REVERSE (thread_state _ _)`],
        Cases_on `thread_state s xs` >> gvs [with_fst_def, MAP_REVERSE])
  (* thread_state body *)
  >>~- ([`thread_state _ _`],
